...
<div class="card card-compact w-full bg-neutral shadow-x1">
	<figure style="height: 300px">
		<img class="h-full w-full object-cover" src={machine.image.src} alt="$assets/placeholder_machine.png" />
	</figure>
		...
		<table class="table table-zebra w-full">
			<tbody>
				<tr>
					<th>用途</th>
                    <td class="text-right">
                        {#each machine.usage as usage}
                            <div class="badge badge-outline">{usage}</div>
                        {/each}
                    </td>
				</tr>
				<tr>
